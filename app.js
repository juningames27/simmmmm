...
async function loadBooks() {
    ...
    datalist.innerHTML = "";
    books.forEach(b => {
        if (b.status === "Disponível") datalist.innerHTML += `<option value="${b.id} | ${b.title}">`;
        tableBody.innerHTML += `<tr><td>${b.id}</td><td>${b.title}</td><td>${b.author}</td><td><span class="badge ${b.status === 'Alugado' ? 'bg-danger' : 'bg-success'}">${b.status}</span></td><td><button onclick="openEditBookModal('${b.id}', '${b.title}', '${b.author}')" class="btn-edit-row">✏️</button><button onclick="deleteBook('${b.id}')" class="btn-delete-row">🗑️</button></td></tr>`;
    });
}
...
document.getElementById("form-book").onsubmit = async (e) => {
    ...
    await fetch(`${API_URL}/books`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title, author }) });
...
document.getElementById("form-loan").onsubmit = async (e) => {
    ...
    await fetch(`${API_URL}/loans`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    e.target.reset(); loadLoans();
};
...
async function openModal(loanId) {
    ...
    document.getElementById("modal-details").innerHTML = `<p><strong>Aluno:</strong> ${loan.studentName}</p><p><strong>Telefone:</strong> ${loan.phone || '---'}</p><p><strong>Escola:</strong> ${loan.school}</p><p><strong>Série:</strong> ${loan.grade}</p><p><strong>Livro:</strong> ${loan.bookTitle}</p><p><strong>Data Entrega:</strong> ${loan.returnDate}</p>`;
...
async function updateLoan() {
    ...
    await fetch(`${API_URL}/loans/${currentLoanId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    closeEditLoanModal(); loadLoans();
}
