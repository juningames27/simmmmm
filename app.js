...
async function loadLoans() {
    ...
    loans.forEach((l) => {
        ...
        const isLate = new Date(parts[2], parts[1] - 1, parts[0]).getTime() < today;
        
        tbody.innerHTML += `
        <tr onclick="openModal('${l.id}', '${l.studentName}', '${l.bookTitle}', '${l.returnDate}')" style="cursor:pointer">
            <td>${l.studentName}</td>
            <td>${l.phone || "---"}</td> <td>${l.school}</td>
            <td>${l.grade || "---"}</td>
            <td>${l.bookTitle}</td>
            ...
            <td>
                ...
                <button class="btn-ver" style="background: #fff; border-radius: 4px; padding: 2px 5px; color: #000; font-size: 12px; border: none;">🔍 Detalhes</button>
            </td>
        </tr>`;
    });
}
...
document.getElementById("form-loan").onsubmit = async (e) => {
    ...
    const res = await fetch(`${API_URL}/loans`, {
        ...
        body: JSON.stringify({
            ...
            phone: document.getElementById("loan-phone").value, // Adicione esta linha aqui
